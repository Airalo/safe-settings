...
module.exports = class Labels extends Diffable {
  ...
  remove (existing) {
    // Note: Exclusion filtering is now handled at the comparison level in diffable.js
    // so this method should only be called for labels that should actually be deleted
    if (this.nop) {
      return Promise.resolve([
