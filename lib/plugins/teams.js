...
module.exports = class Teams extends Diffable {
  ...
  comparator (existing, attrs) {
    return existing.slug === _.kebabCase(attrs.name)
  }
  ...
  add (attrs) {
    ...
    return this.github.teams.getByName({ org: this.repo.owner, team_slug: _.kebabCase(attrs.name) }).then(res => {
      existing = res.data
  ...
  toParams (existing, attrs) {
    return {
      team_id: existing.id,
      org: this.repo.owner,
      team_slug: _.kebabCase(attrs.name),
