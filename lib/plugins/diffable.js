    ...
    if (this.entries) {
      ...
      return this.find().then(existingRecords => {
        ...
        if (typeof this.isExcluded === 'function') {
          filteredExistingRecords = existingRecords.filter(existing => !this.isExcluded(existing))
          this.log.debug(`Filtered ${existingRecords.length - filteredExistingRecords.length} excluded items from existing records`)
        }
